...
public class ExportBeanExcel<T> extends PoiExcelBase<T> {
	...
	public int exportBeanExcel(int excelVersion, String title, List<String> headersName, List<String> headersId,
		...
		int flag = 0;
		// 写入excel
		Workbook wb = writeInExcel(excelVersion, title, headersName, headersId, dtoList, out, null);
		try {
			wb.write(out);
			flag = 1;
	...
	public int exportStyleBeanExcel(int excelVersion, String title, List<String> headersName, List<String> headersId,
		...
		int flag = 0;
		// 写入excel
		Workbook wb = writeInExcel(excelVersion, title, headersName, headersId, dtoList, out, styleUtil);
		try {
			wb.write(out);
			flag = 1;
		} catch (FileNotFoundException e) {
	...
	public int exportStyleBeanExcel(int excelVersion, String title, List<String> headersName, List<T> dtoList,
		...
		int flag = 0;
		// 写入excel
		Workbook wb = writeInExcel(excelVersion, title, headersName, null, dtoList, out, styleUtil);
		try {
			wb.write(out);
			flag = 1;
		} catch (FileNotFoundException e) {
	 ...
	 */
	private void writeInPageExcel(Workbook wb, Sheet sheet, List<String> headersId, List<T> dtoList,
			StyleInterface styleUtil, int pageNum, int pageSize) {
