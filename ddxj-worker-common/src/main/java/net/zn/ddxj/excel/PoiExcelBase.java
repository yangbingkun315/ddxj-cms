...
public abstract class PoiExcelBase<T> implements PoiInterface<T> {
	/*
	 * EXCEL版本支持：03与07 0 EXCEL_VERSION_03 1 EXCEL_VERSION_07 （普通JavaBean） -1 导出EXCEL文件（无分页） -2 分页导出默认文件 （Map结构） -5
	 * 导出默认样式EXCEL文件
	 */
	...
	@Override
	public int exportStyleBeanExcel(int excelVersion, String title, List<String> headersName, List<T> dtoList,
			OutputStream out, StyleInterface styleUtil) {
		return 0;
	}
	...
	@Override
	public Sheet exportPageContentBeanExcel(Workbook wb, Sheet sheet, List<String> headersId, List<T> dtoList,
			StyleInterface styleUtil, int pageNum, int pageSize) {
		return sheet;
	}

	@Override
	public Sheet exportPageContentBeanExcel(Workbook wb, Sheet sheet, List<T> dtoList, StyleInterface styleUtil,
