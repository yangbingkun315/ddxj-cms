...
public class ExportMapExcel<T> extends PoiExcelBase<T> {
	 ...
	 */
	@Override
	public Sheet exportPageContentMapExcel(Workbook wb, Sheet sheet, List<String> headersId,
			List<Map<String, Object>> dtoList, StyleInterface styleUtil, int pageNum, int pageSize) {
		// 字段---标题的字段
		Map<Integer, String> titleFieldMap = getTitleFieldMap(headersId);
		// 表格一行的字段的集合
		Collection zdC = titleFieldMap.values();
	...
	private void writeInPageExcel(Iterator<Map<String, Object>> titleFieldIt, Sheet sheet, Collection zdC,
			CellStyle dataStyle, int pageNum, int pageSize) {
	 ...
	 * 抽象出写入数据层---有标题字段匹对
	 */
	private void doWriteInExcel(Iterator<Map<String, Object>> titleFieldIt, int zdRow, Sheet sheet, Collection zdC,
			CellStyle dataStyle) {
	 ...
	 */
	@Override
	public Sheet exportPageContentMapExcel(Workbook wb, Sheet sheet, List<Map<String, Object>> dtoList,
			StyleInterface styleUtil, int pageNum, int pageSize) {
		// 表格一行的字段的集合
		Iterator<Map<String, Object>> labIt = dtoList.iterator();// 总记录的迭代器
		// 获取自定义的数据样式：
		CellStyle dataStyle = null;
	...
	// 无标题字段匹对--分页
	private void writeInPageExcel(Iterator<Map<String, Object>> titleFieldIt, Sheet sheet, CellStyle dataStyle,
	 ...
	 */
	private void writeInExcelWithoutField(Iterator<Map<String, Object>> titleFieldIt, Sheet sheet, CellStyle dataStyle,
			int zdRow) {
		while (titleFieldIt.hasNext()) {// 记录的迭代器，遍历总记录
			Map<String, Object> mapTemp = titleFieldIt.next();// 拿到一条记录
			zdRow++;
