...
public interface PoiInterface<T> {
	...
	int exportStyleBeanExcel(int excelVersion, String title, List<String> headersName, List<T> dtoList,
	...
	Sheet exportPageContentBeanExcel(Workbook wb, Sheet sheet, List<String> headersId, List<T> dtoList,
			StyleInterface styleUtil, int pageNum, int pageSize);

	// 分页导出Bean结构自定义样式Excel文件----数据体--没有标题栏字段匹配--默认导出dtolist的所有数据
	Sheet exportPageContentBeanExcel(Workbook wb, Sheet sheet, List<T> dtoList, StyleInterface styleUtil, int pageNum,
			int pageSize);


	// 分页导出Map结构自定义样式Excel文件----数据体--根据headersId筛选要导出的字段
	Sheet exportPageContentMapExcel(Workbook wb, Sheet sheet, List<String> headersId, List<Map<String, Object>> dtoList,
			StyleInterface styleUtil, int pageNum, int pageSize);

	// 分页导出Map结构自定义样式Excel文件----数据体--没有标题栏字段匹配，数据体dtoList需要使用treemap。--默认导出dtolist的所有数据
	Sheet exportPageContentMapExcel(Workbook wb, Sheet sheet, List<Map<String, Object>> dtoList,
			StyleInterface styleUtil, int pageNum, int pageSize);
	...
	int exportStyleMapExcel(int excelVersion, String title, List<String> headersName, List<Map<String, Object>> dtoList,
			OutputStream out, StyleInterface styleUtil) throws Exception;
}